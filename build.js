...
function buildAlias(name, peers) {
    return [
        `DCNAME=${name}`, `alias dc='docker-compose -p $DCNAME -f $DCNAME-peers.yml'`, `alias dcup='dc stop ; dc rm -f; dc up -d'`, `alias dojoexec='docker exec -it $\{DCNAME\}_dltdojo_1'`, `alias ddj='dojoexec node index.js'`
    ]
}
