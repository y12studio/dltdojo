...
```
$ dcup
$ fabp0 peer channel create -c ch1
$ fabp0 peer channel join -b ch1.block
$ fabp0 sh -c "export CORE_PEER_ADDRESS=fabp1:7051 ; peer channel join -b ch1.block"
$ fabp0 sh -c "export CORE_PEER_ADDRESS=fabp2:7051 ; peer channel join -b ch1.block"

$ fabp0 peer chaincode deploy -C ch1 -n mycc0 -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example02 -c '{"Args":["init","a","100","b","200"]}'
...
$ fabp0 peer chaincode query -C ch1 -n mycc0 -c '{"Args":["query","b"]}'

$ fabp2 peer chaincode deploy -C ch1 -n mycc2 -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example02 -c '{"Args":["init","a","900","b","1200"]}'
$ fabp0 peer chaincode query -C ch1 -n mycc2 -c '{"Args":["query","b"]}'
$ fabp2 peer chaincode invoke -C ch1 -n mycc2 -c '{"Args":["invoke","a","b","99"]}'
$ fabp1 peer chaincode query -C ch1 -n mycc2 -c '{"Args":["query","a"]}'
