...
Dltbuilder.prototype.buildHead = function(title, sep) {
    ...
    Dltbuilder.prototype.buildAlias = function(name, peers) {
        return [
            `DCNAME=${name}`, `alias dc='docker-compose -p $DCNAME -f $DCNAME-peers.yml'`, `alias dcup='dc stop ; dc rm -f; dc up -d'`, `alias dcend='dc stop ; dc rm -f'`, `alias dojoexec='docker exec -it $\{DCNAME\}_dltdojo_1'`, `alias ddj='dojoexec node index.js'`
        ]
    }
...
Dltbuilder.prototype.btcDojoAlias = function() {
    ...
    _.range(peers).forEach(function(e, i, a) {
        ...
        r.push(`alias ${vpid}${i}exec='docker exec -it $\{DCNAME\}_${vpname}_1'`)
...
Dltbuilder.BuildMethod = function(argv) {
    ...
    } else if (argv.bulkuser && argv.num && argv.prefix) {
        DU.logj(Dltbuilder.BuildBulkUserWithBitconHdKey(argv.num, argv.prefix))
    } else {
        console.log(argv)
    }
}

Dltbuilder.BuildBulkUserWithBitconHdKey = function(n, usernamePrefix) {
    ...
    _.times(n, (i) => {
        ...
        var username = `${usernamePrefix}${index}dltdojo`
        var password = pkey.toWIF(Networks.testnet).substring(0,16)
        accounts.push({
            username: username,
            address: pkey.toAddress(Networks.testnet).toString(),
            password: password,
            script: `useradd -m ${username}; echo -e "${password}\\n${password}" | passwd ${username} ; echo -e "alias ll='ls -al'\\nalias btcp${index}='docker exec -it dltdojo_btcp${index}_1'" > /home/${username}/.bashrc`
        })
    })
    return {
