...
Dltbuilder.BuildAlias = function(name) {
    var arr =  [
        `DLTDOJOID=1`,
        `DCNAME=${name}`, `alias dc='docker-compose -p $DCNAME -f peers$DCNAME.yml'`,
    ...
    _.each(['btc','eth','fab'],(r)=>{
        arr.push(`alias ${r}pstop='confirm && dc stop ${r}p$\{DLTDOJOID\}'`)
        arr.push(`alias ${r}pstart='confirm && dc start ${r}p$\{DLTDOJOID\}'`)
    })
    _.each(['btc','eth'],(r)=>{
        arr.push(`alias ${r}p='ddj ${r} ${r}p$\{DLTDOJOID\}'`)
        arr.push(`alias ${r}pexec='docker exec -it $\{DCNAME\}_${r}p$\{DLTDOJOID\}_1'`)
    })
    return arr
}
...
Dltbuilder.BuildBtcDojo = function(img, path, name, peers) {
    ...
    // var arr = [`alias btcp='ddj btc btcp$\{DLTDOJOID\}'`,`alias btcpexec='docker exec -t $\{DCNAME\}_btcp$\{DLTDOJOID\}_1'`]
    // var vpid = 'btcp'
...
Dltbuilder.BuildFabricDojo = function(img, path, name, peers) {
    ...
    var arr = [`alias fabp='docker exec -t $\{DCNAME\}_fabp$\{DLTDOJOID\}_1'`]
...
Dltbuilder.BuildEthDojo = function(img, path, name, peers) {
    ...
    //})
    return {services:dc.services, arr:[]}
}
...
Dltbuilder.BuildBulkUserWithBitconHdKey = function(n, usernamePrefix, dojoname) {
    ...
    _.times(n, i => {
        ...
        var useraddscript = `useradd -m ${username}; echo -e "${password}\\n${password}" | passwd ${username}\nusermod -s /bin/bash ${username};  usermod -aG docker ${username}\nsed -e 's/DLTDOJOID=1/DLTDOJOID=${index}/g' alias${dojoname}.sh >> /home/${username}/.bashrc\ncp peers${dojoname}.yml /home/${username}/\n`
        ...
        scriptArr.push(useraddscript)
        var msg = `#${index}\r\nWelcome to TiGuan Event ${dojoname}. DLTDOJOID=${index} username=${username} password=${password} please refer to https://y12ji.com/p/dltdojo2017/ for the IP address.`
        messageArr.push(msg)
        accounts.push({
            username: username,
            ...
            password: password,
            message: msg,
            script: useraddscript
        })
    })

    // write to /tmp
    fs.writeFileSync('/tmp/djsetup.sh', scriptArr.join('\n'))
    fs.writeFileSync('/tmp/userinfo.txt', messageArr.join('\r\n'))
    return {
        dojoname: dojoname,
