...
Dlteth.LatestReceiptInfo = function(accountAddress, accountPassword, compileResult) {
    ...
    var lowestBlock = highestBlock - 30
    if (lowestBlock < 0) {
        lowestBlock = 0
    }
...
Dlteth.EthMethod = function(argv) {
    ...
    switch (argv.method) {
        ...
        case 'txinfo':
            ...
            break;
        case 'tiguan5coin':
            var source = fs.readFileSync('./lib/Tiguan5Coin.code.json', "utf-8");
            var cr = JSON.parse(source);
            if (argv.contractinfo) {
                console.dir(cr)
            } else if (argv.new && argv.address && argv.password) {
                var unlock = web3.personal.unlockAccount(argv.address, argv.password)
                Dlteth.CreateContract(argv.address, argv.password, cr)
            } else if(argv.contract && argv.address){
                var contractAbi = cr.abi
                var instance = web3.eth.contract(contractAbi).at(argv.contract)
                if (argv.send && argv.password && argv.amount && argv.to) {
                    var unlock = web3.personal.unlockAccount(argv.address, argv.password)
                    instance.send(argv.to, argv.amount, {
                        from: argv.address
                    }, DU.callback)
                } else {
                    var balance = instance.balances(argv.address)
                    DU.log({
